...
class ListByIdVM:ViewModel() {
    ...
    private var _listaProductosBusqueda: MutableLiveData<List<Product>> =
    ...
    //corrutina que llama a la api y carga el listado por usuario
    //necesario para editar o eliminar productos
    suspend fun getListadoProductosByUser(idUsuario:Int){

        viewModelScope.launch(Dispatchers.IO) {

            try {
                //hacemos la llamada a la api
                var response= getRetrofit().create(ApiService::class.java).getProductsListByUser(idUsuario)
